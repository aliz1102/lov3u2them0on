...
async function sendToDiscord(payloadText) {
  try {
    ...
    // NOTE: Discord generally does not like no-cors + some environments may block it.
...
$(document).ready(function () {
  ...
  function textGenerate() {
    ...
    if (count > 0) {
      for (let i = 1; i <= count; i++) {
        n = n + (a[i] ?? "");
        if (i >= text.length) {
          // if they "type past" the full message, reset
          $("#txtReason").val("");
          n = "";
          break;
        }
  ...
  $("#yes").on("click", async function () {
    ...
    await Swal.fire({
      ...
      // start immediately when popup opens
      didOpen: () => {
    ...
    // Popup #2: real reason textarea (required, no skip)
    let realAnswer = "";
